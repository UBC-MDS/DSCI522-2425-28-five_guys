...
When plotting bike usage against features such as the hour of the day, seasonality, temperature, and holiday status, we noticed non-uniform patterns in their distribution with some categories having little overlap on each other [@fig-season-hourly, @fig-season-temp-count, @fig-holiday_dist]. We observed that there is a spike in the number of bikes rented in summer seasons and also there is a spike in the morning and evening of each day observed [@fig-season-hourly]. This suggests that these features might serve as strong predictors on bike usage. 
